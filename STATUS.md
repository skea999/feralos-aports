...
- NEW dms-shell 1.6.2: Quickshell-based desktop shell (Material 3), musl
  native Go build — replaces nix dms-shell; depends accountsservice +
  quickshell (generic, NO compositor hard dep)
- sources: BOTH upstream tarballs — DMS v1.6.2 tag + dank-qml-common at
  pinned commit 26396ce4 (submodule, verified via `git ls-tree HEAD`);
  prepare() places it at repo root (name from .gitmodules) and runs
  `make sync-shell` (embeds QML into binary via shellembed, `-tags withshell`)
- build: go 1.26 (alpine:latest has 1.26.8 ≥ go.mod 1.26.5), Arch flags
  (-tags distro_binary,withshell -trimpath -buildmode=pie -mod=readonly
  -modcacherw, ldflags -s -w -linkmode=external -X main.Version=v1.6.2),
  GOTOOLCHAIN=local; dgop = embedded Go library (go.mod dep, NOT a runtime
  binary — no dgop package needed)
- packaging: `make install` with seds (Arch parity) — strip
  update-desktop-database / gtk-update-icon-cache, PLUS install-systemd
  ($HOME-based unit, Alpine has no systemd; Arch redirects it instead)
- contents: usr/bin/dms, 3 desktop entries, bash/zsh/fish completions
  (generated by running the built binary at package() time — root-safe
  allowlist), icon, LICENSE, README + docs/ (12 files); QML NOT shipped
  separately — embedded since v1.6 (share/quickshell/dms is pre-1.6 only)
- optdepends: cava + matugen DROPPED (absent on alpine:latest, verified via
  apk search; informational anyway); kept networkmanager, qt6-qtmultimedia,
  power-profiles-daemon, cups-pk-helper, i2c-tools, wtype
- rehearsal (docker alpine:latest, real client flow: keyring from Pages →
  local repo): abuild -Fr 43 s, `dms version` = v1.6.2, headless completion
  OK, embedded QML verified via strings, isolated-root smoke OK (134 pkgs)
- CI fixes (ordering, mechanical): build step now multi-pass — abuild
  re-indexes local repo after each success, deferred packages retried until
  graph exhausted (alphabetical ≠ topological: dms-shell → quickshell →
  cli11); shared-root smoke install now uses smoke-repositories file so
  local-pkg runtime deps resolve
- CI green: build + deploy + client-test (dms-shell installs from Pages,
  signature verified, deps resolve from same repo)
